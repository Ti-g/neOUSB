...
SRMega32::SRMega32(){
        open(0x16C0, "www.pjradcliffe.wordpress.com/open-usb-io", 1503, "Open-USB-IO");	
	reg_map["SREG"] = 0x5F;
	reg_map["SPH"] = 0x5E;
	reg_map["SPL"] = 0x5D;
	reg_map["OCR0"] = 0x5C;
	...
	//16 bit aliases, the firmware knows these addresses and treats them as 16 bit.
	reg_map["TCNT1"] = 0x4C;
	reg_map["OCR1A"] = 0x4A;
	reg_map["OCR1B"] = 0x48;
	reg_map["ICR1"]  = 0x46;
...
void SRMega32::WriteEE(int addr, int data)
{
	unsigned char report[6];
	report[0] = 2;
	report[1] = 3;
...
unsigned char* SRMega32::ReadEE(int addr)
{
	static unsigned char report[6];
	report[0] = 2;
	report[1] = 2;
	report[2] = addr >> 8;
	report[3] = addr & 0xff;
...
/*================= user commands ========================================
 ...
 PURPOSE - send data to the users code.

*/
unsigned char* SRMega32::user_command(int u08A, int u16A, int u16B)
{
	static unsigned char report[6];
	report[0] = 4;
	report[1] = u08A ;
...
/*================= read_io_3 ========================================
 ...
 PURPOSE - read PORTA-D.

*/
unsigned char* SRMega32::read_io_3()
{
	static unsigned char report[6];
        report[0] = 3;
	sendreport(&report[0], 6);
...
/*================= read_ram_5 ========================================
 ...
 PURPOSE - read 5 bytes of RAM.

*/
unsigned char* SRMega32::read_ram_5(int ctrl, int addr, int data)
{
	static unsigned char report[6];
        report[0] = 5;
	report[1] = ctrl;
...
/*================= write_ram ========================================
 ...
 PURPOSE - write one or 2 bytes of RAM.

*/
void SRMega32::write_ram(int ctrl, int addr, int data)
{
	unsigned char report[6];
        report[0] = 6;
	report[1] = ctrl;
...
/*================= write_ram_ee ========================================
 ...
 PURPOSE - write one or 2 bytes of RAM or EEPROM.

*/
void SRMega32::write_ram_ee(int ctrl, int addr, int data, bool ee)
{
	unsigned char report[6];
        if (ee)
             {	report[0] = 2;
                if (ctrl == 2) // 2 bytes to write.
	             report[1] = 7;
	        else report[1] = 3;
#ifdef _WIN32
                Sleep(30) ; //Win32 Sleep is in ms.
#else
...
double SRMega32::PWMSetup(int pin, double frequency)
{
	...
	if(pin < 1 || pin > 4) // pin = PWM number, 1-4 or out of range.
		return -1;
	...
	{	if(pin == 1)
		...
		if(pin == 1 || pin == 4){
			//pwm 1 and 4 can only take on fixed frequencies, we round to the nearest.
			...
			if(frequency < fPCK/65536)
				frequency = fPCK/65536 ;
			if(frequency > fPCK/4)
				frequency = fPCK/4 ;
			//--- calculate register values and send.
			int TOP = (int)round(fPCK/frequency - 1);
			unsigned char TCCR1A, TCCR1B;
			TCCR1A = (*this)["TCCR1A"];
			TCCR1B = (*this)["TCCR1B"];
			(*this)["TCCR1A"] = (TCCR1A & 0xF0) | 0x02;  //set WGM11
			(*this)["TCCR1B"] = (TCCR1B & 0xE0) | 0x19 ; //set WGM13, WGM12 and CS10
			(*this)["ICR1"]   = TOP;
...
{	
	(*this)["ADMUX"] = pin & 0x1F;                 // select ADC pin to use.
